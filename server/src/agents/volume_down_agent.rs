//! Volume Down Agent - 音量减小指令 Agent
//!
//! 处理 agent.volume.down intent，通过 tool_client 调用客户端音量减小指令
...
pub struct VolumeDownAgent;

impl Default for VolumeDownAgent {
    fn default() -> Self {
        Self::new()
    }
}

impl VolumeDownAgent {
    pub fn new() -> Self {
        Self
    }

    /// 检查是否有音量减小工具可用（tools 或 offline_tools 中有 reduce_volume）
    fn has_volume_down_capability(tools: &[Tool], offline_tools: &[String]) -> bool {
        // 检查 tools 中是否有 reduce_volume
        let has_in_tools = tools.iter().any(|t| {
            let name = t.function.name.to_lowercase();
            name.contains("reduce_volume") || name.contains("decrease_volume") || name.contains("volume_down")
        });

        // 检查 offline_tools 中是否有 reduce_volume
        let has_in_offline = offline_tools.iter().any(|name| {
            let name_lower = name.to_lowercase();
            name_lower == "reduce_volume" || name_lower == "decrease_volume"
        });
...
#[async_trait]
impl Agent for VolumeDownAgent {
    fn id(&self) -> &str {
        "agent.volume.down"
    }

    fn intents(&self) -> Vec<&str> {
        vec!["agent.volume.down"]
    }

    async fn run(&self, ctx: AgentContext, handles: AgentHandles<'_>) -> anyhow::Result<()> {
        // 检查是否有音量减小能力（tools 或 offline_tools 中有 reduce_volume）
        if !Self::has_volume_down_capability(&ctx.tools, &ctx.offline_tools) {
