...
impl NavigationAgent {
    ...
    ///
    /// # 工具过滤规则
    /// - 关键字过滤: 包含 "map" 或 "navigat" 关键字的工具（不区分大小写）
    ///
    /// # 内置工具注入
    /// - `math`: 数学计算工具（用于路线计算等）
    /// - `search_web`: 搜索工具（仅在 `enable_search` 为 true 时存在，用于搜索地点信息）
    ///
    /// # 工具合并流程
    /// 1. 从共享工具中筛选包含指定关键字的工具
    /// 2. 追加内置工具 math 和 search_web（如果存在）
    fn merge_tools(shared_tools: &[Tool]) -> Vec<Tool> {
        let (mut tools, mut names) = select_tools_by_keywords(shared_tools, &["map", "navigat"]);
        append_tool_by_name(&mut tools, &mut names, shared_tools, "math");
        append_tool_by_name(&mut tools, &mut names, shared_tools, "search_web");
        tools
    }

    /// 检查是否有可用的导航工具
    fn has_navigation_tools(tools: &[Tool]) -> bool {
        tools.iter().any(|t| {
            let name = t.function.name.to_lowercase();
            name.contains("map") || name.contains("navigat")
        })
    }
    ...
    async fn process_tool_call(
        ...
        let call_id = ensure_tool_call_id(tool_call, "navigation", loop_index, call_index);
...
impl Agent for NavigationAgent {
    ...
    async fn run(&self, ctx: AgentContext, handles: AgentHandles<'_>) -> anyhow::Result<()> {
        ...
        // 检查是否有可用的导航工具，没有则 reject（不保存到上下文，避免污染后续对话）
        if !Self::has_navigation_tools(&tools) {
            let reject_msg = match language.split('-').next().unwrap_or(language) {
                "zh" => "抱歉，导航功能暂时不可用。",
                "ja" => "申し訳ございませんが、現在ナビゲーション機能はご利用いただけません。",
                "ko" => "죄송합니다. 현재 내비게이션 기능을 사용할 수 없습니다.",
                "es" => "Lo siento, la navegación no está disponible en este momento.",
                "it" => "Mi dispiace, la navigazione non è disponibile al momento.",
                _ => "Sorry, navigation is not available at the moment.",
            };
