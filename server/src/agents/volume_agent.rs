...
pub struct VolumeControlAgent;

impl Default for VolumeControlAgent {
    fn default() -> Self {
        Self::new()
    }
}

impl VolumeControlAgent {
    pub fn new() -> Self {
        Self
    }
    ...
    ///
    /// # 工具过滤规则
    /// - 关键字过滤: 包含 "volume" 或 "device" 关键字的工具（不区分大小写）
    ///
    /// # 内置工具注入
    /// - `math`: 数学计算工具（用于音量百分比计算等）
    ///
    /// # 工具合并流程
    /// 1. 从共享工具中筛选包含指定关键字的工具
    /// 2. 追加内置工具 math（如果存在）
    fn merge_tools(shared_tools: &[Tool]) -> Vec<Tool> {
        let (mut tools, mut names) = select_tools_by_keywords(shared_tools, &["volume", "device"]);
    ...
    async fn process_tool_call(
        ...
        let call_id = ensure_tool_call_id(tool_call, "volume", loop_index, call_index);
...
#[async_trait]
impl Agent for VolumeControlAgent {
    fn id(&self) -> &str {
