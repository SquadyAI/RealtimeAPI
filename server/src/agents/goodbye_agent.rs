...
use super::tool_utils::{UNSUPPORTED_TOOL_TEXT, collect_tool_names, select_tools_by_keywords};
...
const MAX_AGENT_LOOPS: usize = 2;

pub struct GoodbyeAgent;

impl Default for GoodbyeAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
impl GoodbyeAgent {
    ...
    ///
    /// # 工具过滤规则
    /// - 关键字过滤: 包含 "goodbye", "end", 或 "exit" 关键字的工具（不区分大小写）
    ///
    /// # 内置工具注入
    /// - 无（告别场景不需要额外工具）
    ///
    ...
    fn merge_tools(shared_tools: &[Tool]) -> Vec<Tool> {
        let (tools, _names) = select_tools_by_keywords(shared_tools, &["goodbye", "end", "exit"]);
        tools
    }
    ...
    async fn process_tool_call(
        ...
        let call_id = ensure_tool_call_id(tool_call, "goodbye", loop_index, call_index);
...
#[async_trait]
impl Agent for GoodbyeAgent {
    fn id(&self) -> &str {
        "agent.conversation.end"
    }

    fn intents(&self) -> Vec<&str> {
        vec!["agent.conversation.end", "say_goodbye"]
    }
